...
var DEFAULT_SETTINGS = {
  ...
  groceryMode: "replace",
  sendToDashboardShopping: true,
  dashboardShoppingList: "Groceries",
...
var RecipeManagerSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Send to MERIDIAN shopping list").setDesc("When the MERIDIAN dashboard is installed, generated groceries go straight into its shopping list instead of a separate note. Turn off to always write the note below.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.sendToDashboardShopping).onChange(async (value) => {
        this.plugin.settings.sendToDashboardShopping = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("Shopping list name").setDesc("Which MERIDIAN shopping list generated groceries are added to.").addText(
      (text) => text.setPlaceholder("Groceries").setValue(this.plugin.settings.dashboardShoppingList).onChange(async (value) => {
        this.plugin.settings.dashboardShoppingList = value.trim() || "Groceries";
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("Grocery list note").setDesc("Path of the note the grocery list is written to when not sending to the MERIDIAN shopping list.").addText(
...
var GroceryModal = class extends import_obsidian9.Modal {
  ...
  async generate() {
    ...
    this.close();
    if (this.plugin.settings.sendToDashboardShopping) {
      const dash = dashboardShoppingApi(this.plugin.app);
      if (dash) {
        const listName = this.plugin.settings.dashboardShoppingList || "Groceries";
        try {
          const added = await dash.importGroceryMarkdown(markdown, listName);
          new import_obsidian9.Notice(
            added > 0 ? `Added ${added} item${added === 1 ? "" : "s"} to your ${listName} shopping list.` : `Nothing new to add \u2014 those items are already on your ${listName} list.`
          );
          return;
        } catch (e) {
          console.error("recipe-manager: could not send groceries to the dashboard, writing the note instead", e);
        }
      }
    }
...
function dashboardShoppingApi(app) {
  var _a, _b;
  const plugin = (_b = (_a = app.plugins) == null ? void 0 : _a.plugins) == null ? void 0 : _b["meridian-dash"];
  const api = plugin == null ? void 0 : plugin.api;
  if (api && typeof api.importGroceryMarkdown === "function") {
    return { importGroceryMarkdown: api.importGroceryMarkdown.bind(api) };
  }
  return null;
}
